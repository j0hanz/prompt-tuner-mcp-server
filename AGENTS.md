...
- `src/`: server source
  ...
  - `src/cli.ts`: CLI parsing, logging bootstrap, shutdown handling
  - `src/server.ts`: MCP server + stdio transport; registers tools/resources/prompts
  - `src/tools.ts`: tool implementations (`fix_prompt`, `boost_prompt`)
  - `src/schemas.ts`: Zod input/output schemas
  - `src/config.ts`: environment config + constants
  - `src/types.ts`: shared types and error codes
  - `src/lib/`: shared utilities
    - `src/lib/llm.ts`: providers, retry, and runtime
    - `src/lib/errors.ts`: error shaping + logger
    - `src/lib/prompt-utils.ts`: prompt wrapping + output normalization
    - `src/lib/telemetry.ts`: diagnostics channel hooks
